        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.Div(
                            [html.H6("Accumulate trade volume ", className="graph__title")]
                        ),
                        dcc.Graph(id="stock-graph-accumulate_trade_volume"),
                    ],
                    className="one-half column",
                ),
            ],
...
@app.callback(
    Output("stock-graph-accumulate_trade_volume", "figure"),
    [
...
def generate_stock_graph_percentage(selected_symbol, _):
    ...
    for group, data_frame in groups:
        ...
        trace = graph_objects.Scatter(
            x=data_frame.ts.tolist(),
            y=data_frame.trade_volume.tolist(),
            marker=dict(color=COLORS[len(data)]),
            name=group,
        )
        data.append(trace)
